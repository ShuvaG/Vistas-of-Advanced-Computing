	...
	for (int i = 0; i < N; i++)
	{
		for (int row = 0; row < n; row++)
		{
			for (int col = 0; col < n; col++)
			{
				...
				{
					sum = matrix[row + 1][col] + matrix[row][col + 1] + matrix[row + 1][col + 1];
					check_matrix(row, col);
				}
				if (row == 0 && col > 0 && col < n - 1) // check top row
				{
					sum = matrix[row][col - 1] + matrix[row][col + 1] + matrix[row + 1][col - 1] + matrix[row + 1][col] + matrix[row + 1][col + 1];
					check_matrix(row, col);
				}
				if (row == 0 && col == n - 1) // check top right corner
				{
					sum = matrix[row][col - 1] + matrix[row + 1][col - 1] + matrix[row + 1][col];
					check_matrix(row, col);
				}
				if (row == n - 1 && col == 0) // checks bottom left corner
				{
					sum = matrix[row - 1][col] + matrix[row - 1][col + 1] + matrix[row][col + 1];
					check_matrix(row, col);
				}
				if (row == n - 1 && col > 0 && col < n - 1) // check bottom row
				{
					sum = matrix[row][col - 1] + matrix[row - 1][col - 1] + matrix[row - 1][col] + matrix[row - 1][col + 1] + matrix[row][col + 1];
					check_matrix(row, col);
				}
				...
				{
					sum = matrix[row - 1][col] + matrix[row - 1][col + 1] + matrix[row][col + 1] + matrix[row + 1][col + 1] + matrix[row + 1][col];
					check_matrix(row, col);
				}
